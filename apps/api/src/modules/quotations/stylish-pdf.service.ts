...
const durationLabel = (start: Date | null, end: Date | null): string => {
  if (!start || !end) return 'As advised';
...
export async function renderStylishQuotationPdf(input: QuotationPdfInput): Promise<Buffer> {
  ...
  const pill = (
    ...
    x: number,
    top: number,
    options: { fill?: string; color?: string; minWidth?: number; height?: number; size?: number } = {},
  ): number => {
    ...
    doc.save().roundedRect(x, top, width, height, height / 2).fill(options.fill ?? PALE_BLUE).restore();
  ...
  const topRule = (top: number, color = NAVY) => {
    doc.moveTo(M, top).lineTo(W - M, top).lineWidth(2).strokeColor(color).stroke();
  ...
  const drawPolaroid = (
    ...
    if (!drawImage(value, -width / 2 + 3, -height / 2 + 3, width - 6, height - 10, 'contain')) {
      ...
      doc.font('Bold').fontSize(12).fillColor(NAVY).text(fallback, -width / 2 + 3, -2, {
        width: width - 6,
        align: 'center',
      });
    }
  ...
  const drawSprig = (x: number, top: number, direction = 1) => {
    ...
    doc.moveTo(x, top + 24).lineTo(x + direction * 10, top).stroke();
  ...
  const pageHeader = () => {
    ...
    doc.path(`M${W} 0 L${W} 70 L${W - 170} 70 C${W - 135} 50 ${W - 95} 51 ${W - 72} 29 C${W - 47} 6 ${W - 22} 12 ${W} 0 Z`).fill();
    doc.restore();
    drawSprig(30, 30, 1);
    drawSprig(W - 30, 30, -1);
    ...
    doc.font('Times-Roman').fontSize(11).fillColor('#1c58bc').text(
      (company?.name ?? 'TRAVEL COMPANY').toUpperCase(),
      155,
      31,
      { width: W - 310, align: 'center', characterSpacing: 2.1, lineBreak: false },
    );
  };

  const pageFooter = (pageIndex: number, pageCount: number) => {
    const top = 769;
    doc.moveTo(M, top).lineTo(W - M, top).lineWidth(0.7).strokeColor('#8ebdff').stroke();
    if (company?.logo) drawImage(company.logo, 46, top + 12, 42, 38, 'contain');
    else {
      doc.font('Bold').fontSize(6).fillColor(GOLD).text('TRAVEL', 47, top + 22, { width: 40, align: 'center' });
      doc.font('Bold').fontSize(5).fillColor(NAVY).text('HOLIDAYS', 47, top + 30, { width: 40, align: 'center' });
    }
    const columns = [
      {
        x: 105,
    ...
    for (const column of columns) {
      doc.font('Body').fontSize(5.5).fillColor(TEAL).text(column.title, column.x, top + 13, {
        width: 130,
        lineBreak: false,
      });
      doc.font('Body').fontSize(4.9).fillColor(INK).text(column.rows.filter(Boolean).join('\n'), column.x, top + 26, {
        width: 135,
        lineGap: 1.1,
      });
    }
    doc.font('Body').fontSize(4.8).fillColor(MUTED).text(
  ...
  const sectionHeading = (title: string, top: number) => {
    doc.font('Bold').fontSize(13).fillColor(NAVY).text(title.toUpperCase(), M + 3, top, {
      width: CONTENT_W,
      lineBreak: false,
    });
    doc.moveTo(M, top + 25).lineTo(M + 85, top + 25).lineWidth(1.5).strokeColor(GOLD).stroke();
  };
  ...
  if (company?.logo) drawImage(company.logo, W / 2 - 45, 36, 90, 82, 'contain');
    ...
    doc.font('Bold').fontSize(10).fillColor(GOLD).text((company?.name ?? 'TRAVEL COMPANY').toUpperCase(), M, 66, {
      width: CONTENT_W,
      align: 'center',
    });
  }
  ...
  doc.font('Body').fontSize(7).fillColor('#ffffff').text(
  ...
  doc.moveTo(W / 2 - 112, 386).lineTo(W / 2 + 112, 386).lineWidth(0.7).strokeColor('#ffffff').stroke();
  ...
  selectedServices.forEach((label, index) => {
    ...
    doc.save().circle(center, 455, 18).lineWidth(0.8).dash(2, { space: 2 }).strokeColor('#ffffff').stroke().restore();
    if (!drawEmoji(coverIcons[index] ?? '⭐', center - 9, 446, 18)) {
      doc.font('Bold').fontSize(12).fillColor(index % 2 ? '#45c7e8' : GOLD).text('◆', center - 12, 447, {
        width: 24,
        align: 'center',
      });
    }
    doc.font('Body').fontSize(5.2).fillColor('#ffffff').text(label, center - 30, 481, {
      width: 60,
      align: 'center',
    });
  });
  doc.save().rect(0, H - 39, W, 39).fillOpacity(0.86).fill('#ffffff').restore();
  doc.font('Bold').fontSize(4.8).fillColor(INK);
  doc.text(`Consultant: ${consultant?.name ?? company?.name ?? '-'}`, 18, H - 23, { width: 180 });
  doc.text(`Phone: ${consultant?.phone ?? company?.phone ?? '-'}`, 208, H - 23, { width: 175, align: 'center' });
  doc.text(`Email: ${consultant?.email ?? company?.email ?? '-'}`, 390, H - 23, { width: 185, align: 'right', ellipsis: true });

  // Overview ---------------------------------------------------------------
  addContentPage();
  doc.font('Times-Roman').fontSize(11).fillColor(NAVY).text(
  ...
  doc.restore();
  doc.font('Body').fontSize(7.6).fillColor(INK).text(
  ...
  doc.font('Body').fontSize(12).fillColor(INK).text('Trip Overview', M + 31, overviewTop + 20);
  pill(`#${input.quotation.quotationNumber.replace(/\D/g, '').slice(-5) || input.quotation.quotationNumber}`, W - M - 108, overviewTop + 15, {
    fill: TEAL,
    color: '#ffffff',
    minWidth: 84,
    height: 24,
  ...
  const overviewRows = [
    ...
    ['🕘', 'DURATION', durationLabel(input.quotation.travelStartDate, input.quotation.travelEndDate), TEAL],
  ...
  const drawOverviewIcon = (kind: string, left: number, top: number, color: string) => {
    ...
    if (kind === 'destination') {
      ...
      doc.moveTo(x - 8, yy - 3).lineTo(x + 8, yy - 3).stroke();
      doc.moveTo(x - 8, yy + 3).lineTo(x + 8, yy + 3).stroke();
    } else if (kind === 'guest') {
      ...
      doc.circle(x, yy, 9).stroke();
      doc.moveTo(x, yy).lineTo(x, yy - 5).stroke();
      doc.moveTo(x, yy).lineTo(x + 4, yy + 2).stroke();
    } else if (kind === 'date') {
      doc.roundedRect(x - 9, yy - 7, 18, 16, 2).stroke();
      doc.rect(x - 9, yy - 7, 18, 5).fill(color);
      doc.moveTo(x - 4, yy + 2).lineTo(x + 5, yy + 2).stroke();
      doc.moveTo(x - 4, yy + 5).lineTo(x + 3, yy + 5).stroke();
    } else {
      doc.roundedRect(x - 8, yy - 5, 16, 13, 2).stroke();
      doc.roundedRect(x - 4, yy - 9, 8, 5, 2).stroke();
      doc.moveTo(x - 8, yy).lineTo(x + 8, yy).stroke();
    }
    doc.restore();
  };
  overviewRows.forEach(([, label, value, color], index) => {
    ...
    drawOverviewIcon(['destination', 'guest', 'duration', 'date', 'travelers'][index] ?? 'travelers', left, top, color);
    doc.font('Body').fontSize(5.2).fillColor(color).text(label, left + 32, top + 1, { width: 118 });
    doc.font('Body').fontSize(7.1).fillColor(INK).text(value, left + 32, top + 17, { width: 118, height: 27 });
  });

  const investmentTop = 435;
  rounded(M, investmentTop, CONTENT_W, 194, '#ffffff', LINE, 10);
  topRule(investmentTop, TEAL);
  doc.font('Body').fontSize(12).fillColor(INK).text('Investment Summary', M + 31, investmentTop + 21);
  ...
  doc.moveTo(M + 29, investmentTop + 54).lineTo(W - M - 195, investmentTop + 54).lineWidth(0.6).strokeColor(LINE).stroke();
  const priceRows = [
  ...
  priceRows.forEach(([label, count, value], index) => {
    ...
    doc.font('Body').fontSize(7).fillColor(MUTED).text(`${label} (x${count})`, M + 51, rowTop, { width: 175 });
    doc.font('Body').fontSize(7).fillColor(INK).text(money(value, input.version.currency), M + 225, rowTop, { width: 70, align: 'right' });
    doc.moveTo(M + 29, rowTop + 15).lineTo(M + 300, rowTop + 15).lineWidth(0.45).strokeColor(LINE).stroke();
  });
  rounded(W - M - 176, investmentTop + 56, 153, 121, TEAL, TEAL, 17);
  doc.font('Body').fontSize(5.5).fillColor('#b9dbe9').text('TOTAL PACKAGE', W - M - 166, investmentTop + 77, { width: 133, align: 'center' });
  doc.font('Body').fontSize(15).fillColor('#ffffff').text(money(input.version.finalAmount, input.version.currency), W - M - 166, investmentTop + 103, { width: 133, align: 'center' });
  doc.font('Body').fontSize(4.8).fillColor('#b9dbe9').text(input.version.taxNote || 'Inclusive of all taxes, excluding TCS', W - M - 166, investmentTop + 151, { width: 133, align: 'center' });
  ...
  if (asNumber(input.version.initialPaymentAmount) > 0) {
    ...
    doc.font('Body').fontSize(9.3).fillColor(INK).text('Secure Your Booking', M + 53, bookingTop + 17);
    doc.font('Body').fontSize(6).fillColor(MUTED).text(`Pay ${money(input.version.initialPaymentAmount, input.version.currency, 2)} to confirm your booking.`, M + 31, bookingTop + 41);
  ...
  if (flightDetails?.outbound?.segments?.length) journeys.push({ label: 'OUTBOUND', journey: flightDetails.outbound });
  if (flightDetails?.returnJourney?.segments?.length) journeys.push({ label: 'RETURN', journey: flightDetails.returnJourney });
  let flightPage = 0;
  if (flightDetails?.include) {
    for (const { label, journey } of journeys) {
      for (const segment of journey.segments ?? []) {
        ...
        doc.font('Bold').fontSize(5.8).fillColor('#ffffff').text(label, M, top + 10, { width: 80, align: 'center' });
        doc.font('Body').fontSize(6.2).fillColor(NAVY).text(
        ...
        doc.moveTo(M + 1, cardTop).lineTo(M + 1, cardTop + cardHeight).lineWidth(2).strokeColor(NAVY).stroke();
        const airlineLogo = segment.airlineId ? input.images?.airlines?.[segment.airlineId] : null;
        if (!drawImage(airlineLogo, M + 18, cardTop + 11, 66, 41, 'contain')) {
          doc.rect(M + 19, cardTop + 12, 64, 39).fill('#f7f7f7');
          doc.font('Bold').fontSize(7).fillColor(RED).text(clean(segment.airlineName) || 'AIRLINE', M + 19, cardTop + 26, { width: 64, align: 'center' });
        }
        doc.font('Bold').fontSize(6).fillColor(INK).text(clean(segment.airlineName) || 'Airline', M + 9, cardTop + 61, { width: sideWidth - 18, align: 'center' });
        doc.font('Body').fontSize(5).fillColor(MUTED).text(clean(segment.flightNumber), M + 9, cardTop + 76, { width: sideWidth - 18, align: 'center' });
        ...
        doc.font('Body').fontSize(5).fillColor(MUTED).text('DEPARTURE', departureX, cardTop + 11, { width: 78, align: 'center' });
        doc.font('Bold').fontSize(7).fillColor(NAVY).text(clean(segment.from) || clean(journey.fromCity) || '-', departureX, cardTop + 22, { width: 78, align: 'center' });
        doc.font('Body').fontSize(5).fillColor(MUTED).text('ARRIVAL', arrivalX, cardTop + 11, { width: 78, align: 'center' });
        doc.font('Bold').fontSize(7).fillColor(NAVY).text(clean(segment.to) || clean(journey.toCity) || '-', arrivalX, cardTop + 22, { width: 78, align: 'center' });
        const arcLeft = mainX + 139;
        const arcRight = mainX + mainW - 139;
        const arcTop = cardTop + 17;
        doc.moveTo(arcLeft, arcTop + 15).bezierCurveTo(arcLeft + 42, arcTop - 2, arcRight - 42, arcTop - 2, arcRight, arcTop + 15).lineWidth(1.4).strokeColor(NAVY).stroke();
        doc.circle(arcLeft, arcTop + 15, 3).fill(NAVY).circle(arcRight, arcTop + 15, 3).fill(NAVY);
        doc.font('Body').fontSize(7).fillColor(NAVY).text('✈', (arcLeft + arcRight) / 2 - 8, arcTop - 1, { width: 16, align: 'center' });
        doc.font('Bold').fontSize(4.5).fillColor(NAVY).text(clean(segment.duration), (arcLeft + arcRight) / 2 - 30, arcTop + 15, { width: 60, align: 'center' });
        doc.moveTo(mainX + 15, cardTop + 54).lineTo(mainX + mainW - 12, cardTop + 54).lineWidth(0.5).strokeColor(LINE).stroke();
        const timing = [
          ['DATE', date(segment.departureDate, true), false],
          ['DEPARTS', clean(segment.departureTime) || '-', true],
          ['DATE', date(segment.arrivalDate, true), false],
          ['ARRIVES', clean(segment.arrivalTime) || '-', true],
        ] as const;
        timing.forEach(([key, value, emphasized], index) => {
          const left = mainX + 17 + index * 92;
          if (index > 0) doc.moveTo(left - 5, cardTop + 62).lineTo(left - 5, cardTop + 94).lineWidth(0.45).strokeColor(LINE).stroke();
          doc.font('Body').fontSize(4.7).fillColor(MUTED).text(key, left, cardTop + 63, { width: 82, align: 'center' });
          doc.font(emphasized ? 'Bold' : 'Body').fontSize(emphasized ? 10 : 5.7).fillColor(emphasized ? NAVY : INK).text(value, left, cardTop + 76, { width: 82, align: 'center' });
        });
        let baggageX = mainX + 31;
        if (segment.cabinLuggage) {
          doc.font('Body').fontSize(6).fillColor(MUTED).text('▥', baggageX - 15, cardTop + 104);
          baggageX += pill(`Cabin ${segment.cabinLuggage}`, baggageX, cardTop + 100, { minWidth: 55, height: 17, size: 5.2 }) + 10;
        }
        if (segment.checkInLuggage) pill(`Check-in ${segment.checkInLuggage}`, baggageX, cardTop + 100, { minWidth: 67, height: 17, size: 5.2 });
        if (segment.notes) doc.font('Body').fontSize(5.2).fillColor(MUTED).text(`Note: ${clean(segment.notes)}`, mainX + 15, cardTop + 128, { width: mainW - 30, height: 22, ellipsis: true });
        flightPage += 1;
      }
    }
  }
  ...
  if (selectedHotels.length) {
    ...
    for (const { hotel, index } of selectedHotels) {
      ...
      doc.moveTo(M + 1, top).lineTo(M + 1, top + 126).lineWidth(2).strokeColor(NAVY).stroke();
      ...
      const infoX = M + 136;
      doc.font('Bold').fontSize(8).fillColor(NAVY).text(hotel.hotelName, infoX, top + 13, { width: 360 });
      pill(hotel.city || 'Destination', infoX, top + 32, { fill: NAVY, color: '#ffffff', minWidth: 58, height: 17, size: 5 });
      const hotelFacts: Array<[string, string, string]> = [
        ['CHECK-IN', date(hotel.checkInDate), '14:00'],
        ['CHECK-OUT', date(hotel.checkOutDate), '12:00'],
        ['ROOM TYPE', hotel.roomType || 'Room open', `${input.quotation.rooms} Room${input.quotation.rooms === 1 ? '' : 's'}`],
      ...
      hotelFacts.forEach(([label, value, sub], factIndex) => {
        ...
        doc.font('Body').fontSize(4.5).fillColor(MUTED).text(label, left + 8, top + 64, { width: 66 });
        doc.font('Bold').fontSize(5.7).fillColor(NAVY).text(value, left + 8, top + 80, { width: 66, height: 14 });
        doc.font('Body').fontSize(4.7).fillColor(MUTED).text(sub, left + 8, top + 98, { width: 66 });
      });
      y += 141;
    }
  }
  ...
  // Day-wise itinerary -----------------------------------------------------
  const sightseeing = input.version.sightseeingDetails as { include?: boolean; days?: SightDay[] } | null;
  const days: SightDay[] = sightseeing?.include && sightseeing.days?.length
    ? sightseeing.days
  ...
  for (const [dayIndex, day] of days.entries()) {
    ...
    for (const activity of activities) {
      ...
      topRule(top, NAVY);
      const imageValue = (activity.imageUrl ? input.images?.itinerary?.[activity.imageUrl] : null)
      ...
      const dayX = M + 195;
      doc.font('Body').fontSize(5).fillColor(MUTED).text('DAY', dayX, top + 18);
      doc.font('Bold').fontSize(22).fillColor(NAVY).text(String(day.dayNumber ?? dayIndex + 1).padStart(2, '0'), dayX, top + 31);
      doc.moveTo(dayX + 51, top + 17).lineTo(dayX + 51, top + 75).lineWidth(0.7).strokeColor('#b8c8dc').stroke();
      doc.font('Bold').fontSize(8.5).fillColor(INK).text(clean(activity.name) || clean(day.title) || `Day ${day.dayNumber}`, dayX + 66, top + 19, { width: 244, height: 35 });
      doc.font('Body').fontSize(5.7).fillColor(MUTED).text(
        `${date(day.date, true)}${day.city ? `  ·  ${day.city}` : ''}`,
        dayX + 66,
        top + 52,
        { width: 244 },
      );
      doc.moveTo(dayX, top + 75).lineTo(W - M - 12, top + 75).lineWidth(0.5).strokeColor(LINE).stroke();
      let metaTop = top + 88;
      if (activity.startTime) {
        doc.font('Bold').fontSize(5.8).fillColor(NAVY).text('Time', dayX, metaTop);
        doc.font('Body').fontSize(6).fillColor(INK).text(`${activity.startTime}${activity.duration ? `  |  ${activity.duration}` : ''}`, dayX + 38, metaTop);
        metaTop += 20;
      }
      const mealText = day.mealsText
        || (day.meals?.breakfast ? `Breakfast (${day.mealMode === 'NO_TRANSFER' ? 'No Transfer' : 'Hotel'})` : null)
      ...
      if (mealText) {
        ...
        doc.font('Body').fontSize(5.8).fillColor(INK).text(mealText, dayX + 54, metaTop, { width: 185 });
        metaTop += 20;
      }
      ...
      const descriptionTitle = clean(activity.name) || clean(day.title) || `Day ${day.dayNumber}`;
      const availableHeight = BODY_BOTTOM - descriptionTop - 11;
      let bodySize = 7.2;
      if (estimatedRichHeight(richDescription, CONTENT_W - 83, bodySize) + 48 > availableHeight) bodySize = 6.3;
      const bodyHeight = Math.min(availableHeight, Math.max(74, estimatedRichHeight(richDescription, CONTENT_W - 83, bodySize) + 47));
      doc.moveTo(M + 1, descriptionTop).lineTo(M + 1, descriptionTop + bodyHeight).lineWidth(2).strokeColor(NAVY).stroke();
      doc.circle(M + 23, descriptionTop + 20, 6).lineWidth(3).strokeColor(NAVY).stroke();
      doc.font('Bold').fontSize(8).fillColor(INK).text(descriptionTitle, M + 48, descriptionTop + 13, { width: CONTENT_W - 60 });
      drawRichLines(richDescription, M + 48, descriptionTop + 36, CONTENT_W - 65, bodySize, INK, 1.38);
      doc.moveTo(M, descriptionTop + bodyHeight).lineTo(W - M, descriptionTop + bodyHeight).lineWidth(0.7).strokeColor('#b7c5d8').stroke();
      activityPage += 1;
    }
  }
  ...
  if (serviceRows.length || addOns.length || input.version.includeVisa) {
    ...
    for (const [rowIndex, { service, index }] of serviceRows.entries()) {
      ...
      doc.font('Bold').fontSize(4.8).fillColor('#ffffff').text(serviceLabel(service.serviceType).toUpperCase(), tabX, top + 10, { width: tabWidth, align: 'center' });
      const textX = imageLeft ? M + 188 : M + 20;
      const textW = CONTENT_W - 210;
      doc.font('Bold').fontSize(10).fillColor(NAVY).text(service.name, textX, top + 31, { width: textW });
      let tagX = textX;
      if (service.city) tagX += pill(service.city, tagX, top + 53, { minWidth: 64, height: 18, size: 5 }) + 8;
      if (asNumber(service.quantity) > 0) pill(`${clean(service.quantity)}`, tagX, top + 53, { fill: '#fff2d2', color: '#a16a00', minWidth: 40, height: 18, size: 5 });
      doc.font('Body').fontSize(6.1).fillColor(MUTED).text(clean(service.description) || clean(service.notes), textX, top + 92, { width: textW, height: 28, ellipsis: true });
      y += rowHeight + 9;
    }

    const hasVisaCard = input.version.includeVisa && !addOns.some(({ service }) => /visa/i.test(service.name));
    if (addOns.length || hasVisaCard) {
      if (y + 135 > BODY_BOTTOM) addContentPage();
      sectionHeading('Add-On Services', y);
      y += 42;
      const cards: Array<{ name: string; description: string; amount: unknown; index: number | null }> = [
        ...
        ...(hasVisaCard ? [{
          ...
          description: [input.version.visaDestination, input.version.visaType].filter(Boolean).join(' · '),
      ...
      for (const [cardIndex, card] of cards.entries()) {
        ...
        doc.font('Bold').fontSize(5.5).fillColor('#ffffff').text(String(cardIndex + 1), M + 13, y + 13, { width: 14, align: 'center' });
        doc.font('Bold').fontSize(7).fillColor(NAVY).text(card.name, M + 43, y + 13, { width: 340 });
        pill(money(card.amount, input.version.currency), W - M - 62, y + 8, { minWidth: 50, height: 18, size: 5.2 });
  ...
  const visiblePolicies = policies.map(([title, color, html, rows]) => ({
    title,
    color,
    lines: html
  ...
  if (visiblePolicies.length) {
    ...
    const measuredPolicyHeight = (size: number, lineFactor: number) => visiblePolicies.reduce(
      (total, policy) => total + 46 + estimatedRichHeight(policy.lines, CONTENT_W - 64, size, lineFactor),
      0,
    );
    let policySize = 5.65;
    ...
    for (const [policyIndex, policy] of visiblePolicies.entries()) {
      const bodyHeight = estimatedRichHeight(policy.lines, CONTENT_W - 64, policySize, policyLineFactor);
      const sectionHeight = 46 + bodyHeight;
      if (y + sectionHeight > BODY_BOTTOM) addContentPage('Policies');
      const top = y;
      doc.save().fillOpacity(0.08).roundedRect(M, top, CONTENT_W, 34, 8).fill(policy.color).restore();
      doc.circle(M + 31, top + 17, 12).fill(policy.color);
      doc.font('Bold').fontSize(6.5).fillColor('#ffffff').text(String(policyIndex + 1), M + 23, top + 13, { width: 16, align: 'center' });
      doc.font('Bold').fontSize(8.3).fillColor(policy.color).text(policy.title, M + 53, top + 12, { width: CONTENT_W - 74 });
      doc.moveTo(M + 1, top).lineTo(M + 1, top + sectionHeight - 6).lineWidth(2).strokeColor(policy.color).stroke();
      const end = drawRichLines(policy.lines, M + 31, top + 40, CONTENT_W - 62, policySize, INK, policyLineFactor);
      doc.moveTo(M + 29, end + 4).lineTo(M + 170, end + 4).lineWidth(0.7).strokeColor(policy.color).stroke();
  ...
  doc.font('Bold').fontSize(30).fillColor(GOLD).text('THANK YOU', M, 132, { width: CONTENT_W, align: 'center' });
  doc.font('Body').fontSize(9.5).fillColor('#b7c5df').text('for choosing us as your travel partner', M, 176, { width: CONTENT_W, align: 'center' });
  doc.moveTo(227, 203).lineTo(368, 203).lineWidth(2).strokeColor(GOLD).stroke();
  drawDiamond(W / 2, 203, 7, GOLD);
  doc.font('Body').fontSize(7.1).fillColor('#aebbd1').text(
  ...
  contactCards.forEach(([icon, label, value, color], index) => {
    ...
    doc.font('Bold').fontSize(10).fillColor(color).text(icon, left + 25, top + 16, { width: 18, align: 'center' });
    doc.font('Bold').fontSize(5.3).fillColor(color).text(label, left + 51, top + 12);
    doc.font('Body').fontSize(6.5).fillColor('#d4deed').text(value || '-', left + 51, top + 31, { width: width - 65, ellipsis: true });
  });
  drawDiamond(M + 12, 430, 4, GOLD);
  if (company?.address) doc.font('Body').fontSize(6).fillColor('#aebbd1').text(company.address, 220, 420, { width: 185, align: 'center' });
  doc.roundedRect(56, 472, W - 112, 63, 10).lineWidth(1.5).strokeColor(GOLD).stroke();
  const achievements = [
    [company?.tripsSold ?? '-', 'Trips Completed'],
    [company?.operatingSinceYear ? `${new Date().getFullYear() - company.operatingSinceYear}+` : '-', 'Years Experience'],
    ['GST', 'Registered Company'],
  ];
  achievements.forEach(([value, label], index) => {
    const left = 67 + index * 160;
    if (index > 0) doc.moveTo(left - 6, 483).lineTo(left - 6, 524).lineWidth(0.7).strokeColor('#aebbd1').stroke();
    doc.font('Bold').fontSize(14).fillColor(GOLD).text(String(value), left, 484, { width: 145, align: 'center' });
    doc.font('Body').fontSize(5.2).fillColor('#9fb0ce').text(String(label), left, 512, { width: 145, align: 'center' });
  });
  doc.font('Body').fontSize(4.8).fillColor('#647799').text(
    [company?.tan ? `TAN: ${company.tan}` : null, company?.taxRegistrationNumber ? `GST: ${company.taxRegistrationNumber}` : null].filter(Boolean).join('  |  '),
    M,
    568,
    { width: CONTENT_W, align: 'center' },
  );
  doc.rect(127, 583, 341, 3).fill(GOLD);
  doc.font('Bold').fontSize(8.5).fillColor(GOLD).text((company?.name ?? 'TRAVEL COMPANY').toUpperCase(), M, 604, { width: CONTENT_W, align: 'center' });
  doc.font('Body').fontSize(5.5).fillColor('#8192b1').text('Your Trusted Travel Partner', M, 623, { width: CONTENT_W, align: 'center' });
